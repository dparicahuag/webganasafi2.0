...
actions.update.onSubmit = async (e, props) => {
  ...
  const { title, slug, rate, currencySymbol, currencyRate, setFlag, router } = props;
...
export function FileUploadComponent({ setImage, setStateButton }) {
  ...
  const selectFileInput = ({ accept, onFiles, files, getFilesFromEvent }) => {
    const textMsg =
      files.length > 0 ? "Subir de nuevo" : "Seleccionar imagen";
      return (
        <div className="text-center " >
          <label className="btn btn-primary mt-4">
            {textMsg}
            <input
              className="slidersInput01"
              type="file"
              accept={accept}
              multiple
              onChange={(e) => {
          ...
          <br />
          <p className="fa-xs">Tamaño maximo de imagen: 10MB</p>
        </div>
      );
  };
  return (
    <Dropzone
      ...
      inputLabel="Tamano maximo de archivo: 10MB, acepta imagenes"
      styles={{
...
export default function Products(props) {
  ...
  return (
    <AdminContainer>
      ...
      />
      <FileUploadComponent setImage={setImage} setStateButton={setStateButton}/>
      {children(props)}
    </AdminContainer>
  );
}
